...
export default function BookingCards({ category = "" }: BookingCardsProps) {
	...
	const [startDateShuttle, setStartDateShuttle] = useState<Date>(new Date());
	const [endDateShuttle, setEndDateShuttle] = useState<Date>(new Date());

	const [startDateRent, setStartDateRent] = useState<Date>(new Date());
	const [endDateRent, setEndDateRent] = useState<Date>(new Date());
	...
	dropOffLocations = filter(dropOffLocations, (o) => o !== pickUpLocation);
	...
	const getMonth = (date: Date) => moment(date).format("M");
	...
	return (
		<div className="box-bottom-search background-card tw:!z-20">
			...
			<div className="item-search item-search-3 p-2">
					...
					<div className="item-search item-search-3 p-2">
						...
						<div className="box-calendar-date">
							<DatePicker
								...
								maxDate={moment(isShuttle ? startDateShuttle : startDateRent)
									.add(9, "days")
									.toDate()}
								selectsEnd
								todayButton="Hari ini"
