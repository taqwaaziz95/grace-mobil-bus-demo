...
export default function CarsDetails1({
	...
	const [jenis, setJenis] = useState("Mobil + Supir");
	const [transmisi, setTransmisi] = useState("Automatic");
	const [warna, setWarna] = useState("Putih");
	const [durasi, setDurasi] = useState("12 Jam");
	const [bulanan, setBulanan] = useState("6 Bulan");
	const [penjemputan, setPenjemputan] = useState("");
	const [dari, setDari] = useState<Date | null>(new Date());
	const [sampai, setSampai] = useState<Date | null>(new Date());

	const car = find(rawCarsData, (o) => o.id == params?.id);
	...
	function doOrder() {
		...
		const rentDetails = {
			...
			transmisi,
			warna,
			durasi: category === "harian" ? durasi : null,
			bulanan: category === "bulanan" ? bulanan : null,
			penjemputan: category === "bandara" ? penjemputan : null,
			dari:
				(category === "bandara" || category === "harian") && dari
					? dari.toLocaleDateString("id-ID")
					: null,
			sampai:
				category === "bandara" && sampai
					? sampai.toLocaleDateString("id-ID")
					: null,
		...
		// ✅ Format message in Bahasa Indonesia
		let message = `*Halo Admin Gracetrans*,%0A%0A`;
		message += `Saya ingin melakukan pemesanan kendaraan dengan detail sebagai berikut:%0A`;
		message += `*ID Pesanan:* ${orderId}%0A`;
		message += `*Nama Mobil:* ${rentDetails.namaMobil}%0A`;
		message += `*Jenis:* ${rentDetails.jenis}%0A`;
		message += `*Transmisi:* ${rentDetails.transmisi}%0A`;
		message += `*Warna:* ${rentDetails.warna}%0A`;

		if (category === "harian") {
			message += `*Durasi:* ${rentDetails.durasi}%0A`;
		}
		if (category === "bulanan") {
			message += `*Paket Bulanan:* ${rentDetails.bulanan}%0A`;
			message += `*Mulai:* ${rentDetails.dari}%0A`;
		}
		if (category === "bandara") {
			message += `*Penjemputan:* ${rentDetails.penjemputan}%0A`;
			message += `*Dari:* ${rentDetails.dari}%0A`;
			message += `*Sampai:* ${rentDetails.sampai}%0A`;
		}

		message += `%0A*Total Harga:* Rp${rentDetails.harga.toLocaleString("id-ID")}%0A%0A`;
		message += `Mohon konfirmasi ketersediaan. Terima kasih.`;

		// ✅ Randomly choose phone number for load balancing
		const phoneNumbers = ["628119444889", "6282246713465"];
		const randomPhone =
				...
				<div>
					<section className="box-section box-breadcrumb background-body">
						<div className="container">
							<ul className="tw:!py-6 tw:!flex tw:!items-center tw:!text-sm tw:!font-medium">
								<li className="tw:!flex tw:!items-center">
									<Link
										href="/"
										className="tw:!text-[#2389B9] hover:tw:!underline"
									>
										Home
									</Link>
								...
								<li className="tw:!flex tw:!items-center">
									<Link
										...
										className="tw:!text-gray-700 hover:tw:!underline"
									>
										Rental
									</Link>
									<span className="tw:!mx-2 tw:!text-gray-400">
								...
								<div className="box-banner-activities">
									...
									>
										<div className="banner-slide-activity">
											<img src={car?.image} alt="Gracetrans" />
										</div>
										<div className="banner-slide-activity">
											<img src={car?.image} alt="Gracetrans" />
										</div>
